...
|---|---:|---|
| Agent can complete after authorisation | 24 | Code, content, configuration, testing, and documentation can be completed end to end in the repository |
...
| Agent can implement, but completion needs external access, deployment, or elapsed time | 6 | Repository work can be automated; final proof depends on production analytics, account access, or monitoring |
| **Total** | **37** | All numbered action-plan items |

## Agent can complete after authorisation

These 24 items can be assigned directly to a coding/content agent with normal review and deployment gates:
...
| Technical SEO | M-01, M-02, M-10, M-15, M-16, L-02, L-03 | Accurate sitemap dates, SSR counters, heading structure, image optimisation, robots rules, redirect simplification, CSP hardening |
...
These are not all equal-risk. L-03 is security-sensitive and should receive a focused review plus a production smoke test before deployment.
...
U-03 and L-05, plus a fresh crawl, three Lighthouse runs, GSC URL Inspection sampling, schema validation, and lead-event testing.
